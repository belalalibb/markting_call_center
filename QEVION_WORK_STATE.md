...
| Current stage | **P5 COMPLETE** (CP-0008) → P6 Evaluation + Replay + Evidence + Preview (CP-0009) |
| Current objective | P6: `qevion/replay/` deterministic replay (QV-ACC-019: recorded client turns + mock script + deterministic clock → same events/outcome, divergence report), `qevion/eval/` evaluation harness (copilot eval suite QV-EVAL-004: scripted operator personas + fixture uploads with planted conflicts/gaps; runtime eval reuses simulation graders), Config Center Simulate/Gates/Activate panel + Operator Console outbound dial (rebuild web/dist), Evidence pack `evidence/P6/CP-0009/` + registry roll-up, verified Preview URL, real-provider smoke only if an A5 key is present → CP-0009. (Prev P5: simulation contract + SimulatedCustomerTransport + ScenarioRunner with 10 graders + generated persona/adversarial cases; activation gates wired; outbound seam with contact-policy hooks + simulated telephony → CP-0008) |
...
## In progress
- none (CP-0008 closed: record 2d61b3a, tag cp/CP-0008, snapshot manifest)
...
## Exact next action
P6 step 1: create `qevion/replay/recorder.py` (SessionRecording dataclass + `record()` from a finished Session/CaseRun) and `qevion/replay/player.py` (`replay(recording)` → ReplayResult with event/outcome diff) + `tests/test_replay.py` on Activity A/B/C via the simulation runner (commit files before first run).
