...
| Current stage | **P3 in progress** — copilot + runtime API done (196 tests, CI PASS); next `web/` TypeScript app + preview URL → CP-0006 |
...
## In progress
- P3/CP-0006: copilot package DONE (31 tests); `qevion/runtime/{store,app}.py` + `qevion/copilot/api.py` + composition root `qevion/main.py` DONE (14 API tests incl. WS). Next: `web/` (Vite+TS, 3 modes) served from `web/dist` by `qevion/main.py`; preview URL; evidence; CP-0006.
...
## Exact next action
`web/` Vite + TypeScript single app (no framework): `index.html`, `src/main.ts` (router: #/config, #/console, #/admin), `src/api.ts`, modes: Config Center (copilot session → questions/answer/accept/defer → proposal/explanation → publish; activities list, preflight, capabilities, knowledge upload/report/conflicts), Operator Console (sessions list, WS text chat against activity, event stream, handoffs), Admin (tenants, registry, ephemeral test key). Build to `web/dist` (committed), start `uvicorn qevion.main:app --port 8000` in background, `GetServiceUrl` 8000, verify with Playwright console capture. Then evidence `evidence/P3/CP-0006/`, traceability, `checkpoint.sh CP-0006 P3`.
