...
class ToolPipeline:
    ...
    async def run(
        ...
        *,
        ...
        if decl is None:
            return await self._reject(inv, ToolOutcomeStatus.REJECTED_ARGS, "unknown tool", EventType.TOOL_ARGS_INVALID, S.SCHEMA_VALIDATED)
        errs = validate_args(decl.parameters, inv.arguments)
        if errs:
            return await self._reject(inv, ToolOutcomeStatus.REJECTED_ARGS, "; ".join(errs), EventType.TOOL_ARGS_INVALID, S.SCHEMA_VALIDATED)
        ...
        if perm is None or (self.tenant_allowed_tools is not None and inv.tool_id not in self.tenant_allowed_tools):
            return await self._reject(inv, ToolOutcomeStatus.REJECTED_POLICY, "tool not permitted for activity/tenant", EventType.TOOL_POLICY_REJECTED, S.PERMISSION_CHECKED)

        # 4 policy (impact must match declaration)
        if perm.impact != decl.impact.value:
            return await self._reject(inv, ToolOutcomeStatus.REJECTED_POLICY, f"impact mismatch {perm.impact}!={decl.impact.value}", EventType.TOOL_POLICY_REJECTED, S.POLICY_CHECKED)

        # 5 budget
        if not self.budget.allow(perm.impact):
            return await self._reject(inv, ToolOutcomeStatus.REJECTED_BUDGET, "tool budget exhausted", EventType.BUDGET_EXCEEDED, S.BUDGET_CHECKED)

        # 6 dedup
        key = idempotency_key(decl, inv)
        if key and key in self._seen:
            prev = self._seen[key]
            await self.emit(EventType.TOOL_DUPLICATE_IGNORED, {"call_id": inv.call_id, "tool_id": inv.tool_id, "duplicate_of": prev.call_id})
            dup = prev.model_copy(update={"call_id": inv.call_id, "status": ToolOutcomeStatus.DUPLICATE, "step_reached": S.DEDUPLICATED})
        ...
        if perm.confirmation == "confirm_before_execute":
            ...
            if granted is None:
                return ToolOutcome(call_id=inv.call_id, tool_id=inv.tool_id, status=ToolOutcomeStatus.CONFIRMATION_DENIED, error="confirmation pending", step_reached=S.CONFIRMATION)
            if not granted:
                await self.emit(EventType.TOOL_CONFIRMATION_DENIED, {"call_id": inv.call_id})
                return await self._reject(inv, ToolOutcomeStatus.CONFIRMATION_DENIED, "user denied", None, S.CONFIRMATION)
            await self.emit(EventType.TOOL_CONFIRMATION_GRANTED, {"call_id": inv.call_id})

        # 8 execute
        backend = self.backends.get(inv.tool_id)
        if backend is None:
            return await self._reject(inv, ToolOutcomeStatus.FAILED, "no backend bound", EventType.TOOL_EXECUTION_FAILED, S.EXECUTING)
        ...
        try:
            ...
            out = ToolOutcome(call_id=inv.call_id, tool_id=inv.tool_id, status=ToolOutcomeStatus.TIMEOUT, error="timeout", step_reached=S.EXECUTING)
        except Exception as e:  # noqa: BLE001 — adapters must never crash the Core
            out = ToolOutcome(call_id=inv.call_id, tool_id=inv.tool_id, status=ToolOutcomeStatus.FAILED, error=f"{type(e).__name__}: {e}", step_reached=S.EXECUTING)
        ...
        if out.status is ToolOutcomeStatus.UNKNOWN:
            ...
            await self.emit(EventType.TOOL_EXECUTION_FAILED, {"call_id": inv.call_id, "tool_id": inv.tool_id, "status": out.status, "error": out.error})
        else:
            # 10 provenance tag — never stronger than the declaration allows
            prov = out.provenance if _STRENGTH[out.provenance] <= _STRENGTH[decl.result_provenance] else decl.result_provenance
            out = out.model_copy(update={"provenance": prov, "step_reached": S.RETURNED})
            await self.emit(EventType.TOOL_EXECUTION_COMPLETED, {"call_id": inv.call_id, "tool_id": inv.tool_id, "provenance": prov, "latency_ms": out.latency_ms})
            if key:
                self._seen[key] = out
        # 11 returned
    ...
    async def _reject(self, inv: ToolInvocation, status: ToolOutcomeStatus, reason: str, event: str | None, step: ToolPipelineStep) -> ToolOutcome:
        if event:
            await self.emit(event, {"call_id": inv.call_id, "tool_id": inv.tool_id, "reason": reason, "step": step})
        out = ToolOutcome(call_id=inv.call_id, tool_id=inv.tool_id, status=status, error=reason, provenance=Provenance.UNKNOWN, step_reached=step)
        self.history.append(out)
        return out
