...
class S2SSession(Protocol):
    ...
    async def truncate_response(self, response_id: str, audio_end_ms: int) -> None:
        """F-06: drop the provider-side context of a response beyond what the user actually heard
        (`audio_end_ms` of playback). Providers without the concept implement it as a no-op."""
        ...
